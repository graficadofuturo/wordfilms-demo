          ...
          >
            <div className="hidden md:flex gap-8 text-[10px] font-bold uppercase tracking-[0.3em]">
              {currentView === 'home' ? (
                ...
                <button onClick={() => setCurrentView('home')} className="hover:text-zinc-400 transition-colors uppercase tracking-[0.3em]">HOME</button>
              )}
      ...
      <AnimatePresence mode="wait">
        {currentView === 'home' && (
          ...
          {(activeData.sections || DEFAULT_SITE_DATA.sections!).filter(s => s.visible).sort((a, b) => a.order - b.order).map(section => {
            ...
            switch (section.type) {
              case 'hero':
                return (
                  <section key="hero" className="relative h-screen w-full overflow-hidden flex items-center justify-center z-10">
                    {sectionBg}
        <div className="absolute inset-0 z-0">
          {activeData.hero.videoUrl && (
            <video 
              ...
              src={activeData.hero.videoUrl || undefined}
            />
          )}
                  ...
                  return (
                    ...
                    >
                      {line.split(/\s+/).map((word, i) => {
                        ...
                        const getOutlineStyle = (width: string | undefined, color: string | undefined) => {
                          if (!isOutline) return {};
                          const w = parseFloat(formatValue(width) || '1.5') || 1.5;
                        ...
                        return (
                          <div 
                            ...
                            style={{
                              ...
                              letterSpacing: formatValue(style.letterSpacing),
                              ...(!isOutline ? { color: style.color || undefined } : getOutlineStyle(style.outlineWidth, style.color))
                            }}
                          >
                            {word}
        ...
        >
          <ChevronDown size={32} />
              ...
              case 'works':
                return (
                  <section key="works" id="works" className={`${activeData.styles?.spacing || 'py-16 md:py-32'} px-4 md:px-12 bg-transparent relative z-10 w-full overflow-hidden`}>
                    {sectionBg}
        <div className="max-w-screen-2xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12">
            {activeData.portfolio.map((item, index) => (
              ...
              case 'about':
                return (
                  <section key="about" id="about" className={`${activeData.styles?.spacing || 'py-16 md:py-32'} px-4 md:px-12 bg-transparent relative z-10 w-full overflow-hidden`}>
                    {sectionBg}
        <div className="max-w-screen-2xl mx-auto grid grid-cols-1 md:grid-cols-2 md:grid-rows-[max-content_max-content_1fr] gap-y-8 md:gap-x-20 items-start relative z-10">
          ...
          <div className="md:col-start-1 md:row-start-2 -mt-4 md:-mt-6">
            {isEditable ? (
              ...
              case 'expertise':
                return (
                  <section key="expertise" id="expertise" className={`${activeData.styles?.spacing || 'py-16 md:py-32'} px-4 md:px-12 bg-transparent relative z-10 w-full overflow-hidden`}>
                    {sectionBg}
        <div className="max-w-screen-2xl mx-auto relative z-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-6 gap-px bg-zinc-800 border-y border-zinc-800 mt-10 md:mt-20">
            {activeData.services.map((service, index) => {
              ...
              case 'numbers':
                return (
                  <section key="numbers" id="numbers" className={`${activeData.styles?.spacing || 'py-16 md:py-32'} relative z-10 w-full bg-transparent overflow-hidden flex flex-col`}>
                    {sectionBg}
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 md:gap-x-24 lg:gap-x-40 gap-y-12 md:gap-y-16 lg:gap-y-20 max-w-5xl w-full">
                          {(activeData.numbers || []).map((item) => (
                            <div key={item.id} className="flex items-end justify-start md:justify-center gap-4 md:gap-6">
                              <AnimatedCounter
                                value={item.value}
                                className="text-7xl sm:text-8xl md:text-[100px] lg:text-[120px] leading-[0.75] font-light uppercase text-transparent tracking-tight tabular-nums shrink-0"
                                style={{ 
              ...
              case 'clients':
                return (
                  <section key="clients" id="clients" className={`${activeData.styles?.spacing || 'py-16 md:py-32'} px-4 md:px-12 bg-transparent relative z-10 w-full overflow-hidden`}>
                    {sectionBg}
              ...
              case 'contact':
                return (
                  <section key="contact" id="contact" className={`${activeData.styles?.spacing || 'py-16 md:py-32'} px-4 md:px-12 bg-transparent relative z-10 w-full overflow-hidden`}>
                    {sectionBg}
            ...
            {(activeData.contact.items || [
              ...
              <div key={item.id} className="flex flex-col items-center w-full md:w-72">
                {isEditable ? (
                  ...
                  item.link ? (
                    <a href={item.link} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center group">
                      <DynamicIcon name={item.icon} className="text-zinc-500 group-hover:text-zinc-300 transition-colors mb-0 md:mb-8" size={32} />
